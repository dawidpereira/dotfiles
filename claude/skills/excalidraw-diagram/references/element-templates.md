...
Use `"elbowed": true` and `"roundness": {"type": 2}` for clean right-angle routing. This is the default — only use straight arrows (`"elbowed": false`, 2-point `points` array) for short horizontal or vertical connections where an elbow would be overkill.

To add a label on the arrow, bind a text element to it (add the text element's ID to the arrow's `boundElements` and set the text's `containerId` to the arrow's ID). Use a small font size (12-14px) so the label doesn't obscure the arrow line.
...
```json
{
  ...
  "strokeColor": "<arrow color from palette — use the Arrow color, not shape strokes>",
  "backgroundColor": "transparent",
  "fillStyle": "solid",
  "strokeWidth": 1,
  "strokeStyle": "solid",
  ...
  "isDeleted": false,
  "groupIds": [],
  "boundElements": [{"id": "arrow1_label", "type": "text"}],
  "link": null,
  "locked": false,
  "elbowed": true,
  "points": [[0, 0], [60, 0], [60, 50], [118, 50]],
  "roundness": {"type": 2},
...
```json
{
  ...
  "verticalAlign": "middle",
  "containerId": "arrow1",
  "strokeColor": "<body/detail text color from palette>",
  "backgroundColor": "transparent",
  "fillStyle": "solid",
  "strokeWidth": 1,
  "strokeStyle": "solid",
