...
When the user includes "be descriptive", "--descriptive", or similar phrasing, add explanatory text throughout the diagram:

- **Annotations**: Add short free-floating descriptions near each major element explaining what it does or why it matters.
- **Step explanations**: For flows and sequences, add a sentence under each step describing what happens.
- **Legend/key**: Include a small legend if the diagram uses semantic colors or shapes that benefit from labeling.
- **Context notes**: Add brief notes at section boundaries explaining transitions or relationships between sections.

Without this flag, keep text minimal — labels and titles only, letting the visual structure speak for itself.
...
### Stroke Width
- `strokeWidth: 1` — Standard for arrows, lines, dividers, and most connections.
- `strokeWidth: 2` — For shapes (rectangles, ellipses, diamonds).
- `strokeWidth: 3` — Bold. Use sparingly for emphasis on a single key element.
...
### Symmetry First
Center titles and headings horizontally over the content they describe. The diagram's overall composition should be balanced — if content spans from x=100 to x=1200, a title should be centered around x=650, not left-aligned. When a diagram has multiple subsections (e.g., "GNU Stow" and "Manual Symlinks"), center each subsection title over its own content group, and balance the subsections relative to each other so they feel evenly distributed. During the render-view-fix loop, check that titles sit centered above their section and that the diagram doesn't feel lopsided.
...
### Arrow Routing
- **Elbow arrows by default.** Use `"elbowed": true` so arrows route with clean 90-degree segments. Only use straight 2-point arrows for short horizontal or vertical connections.
- **Labels on arrows, not floating.** When an arrow needs a label (like "stow ." or "ln -s"), bind a text element to the arrow rather than placing free-floating text near it. Use a small font (12-14px) for arrow labels so they don't obscure the arrow line. See the Arrow Label template in `references/element-templates.md`.
- **Spread origins.** When multiple arrows leave the same element, each should originate from a distinct point along the element's edge (top, right, bottom) — not all from the same pixel. Stagger start and end points to keep lines visually separated.
...
Save all generated `.excalidraw` files and their rendered PNGs to `~/Documents/Zettelkasten/Inbox/`.
